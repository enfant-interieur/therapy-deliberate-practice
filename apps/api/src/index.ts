...
const mapObjectives = (task: DeliberatePracticeTaskV2["task"], content: ExerciseContentV2) => {
  ...
  const uniqueIds = Array.from(new Set(orderedIds.length ? orderedIds : task.objectives.map((obj) => obj.id)));
  const trimmedIds = uniqueIds.slice(0, 6);
  const objectives: Objective[] = trimmedIds
    .map((id) => objectiveMap.get(id))
    .filter((obj): obj is DeliberatePracticeTaskV2["task"]["objectives"][number] => Boolean(obj))
    .map((obj) => ({
      id: obj.id,
      label: obj.label,
...
app.post("/api/v1/admin/import-exercise", async (c) => {
  ...
  const existingById = data.exercise_overrides?.id
    ? await db.select().from(exercises).where(eq(exercises.id, data.exercise_overrides.id))
    : [];
  const existingBySlug = existingById.length
    ? []
    : await db.select().from(exercises).where(eq(exercises.slug, validated.slug));
